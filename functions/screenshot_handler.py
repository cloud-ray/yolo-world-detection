...
def save_additional_screenshot(obj_id, class_idx, confidence, frame, classes, timestamp, x1, y1, x2, y2):
    """
    ...
    try:
        ...
        screenshot_path = f"{IMAGE_DIRECTORY}/{class_name}_{class_id}_{confidence:.2f}_{obj_id}_{x1}_{y1}_{x2}_{y2}_{timestamp}.png"
...
def check_and_save_screenshot(obj_id, class_idx, confidence, frame, classes, center_x, center_y, box_width, box_height, x1, y1, x2, y2):
    """
    ...
    if obj_id in object_tracker:
        ...
        if obj_data['frame_count'] >= FRAME_COUNT_THRESHOLD:
            ...
            if obj_data['frames_since_last_screenshot'] >= ADDITIONAL_FRAME_THRESHOLD:
                save_additional_screenshot(obj_id, class_idx, confidence, frame, classes, center_x, center_y, box_width, box_height, timestamp, x1, y1, x2, y2)
